...
/*
 * Pull the photo from the linked Google account again.
 *
 * Through photoVisit like the other two, so it shares `photoBusy` — the
 * avatar shows its spinner, the controls disable, and the unsaved-changes
 * guard stays out of the way.
 *
 * `localPreview` is cleared rather than set: with an upload the browser
 * already holds the bytes, so the new photo can be shown instantly, but here
 * the image only exists once the server has fetched it. Anything shown before
 * the response would be a guess at what Google is about to send.
 *
 * The server reports failure as a flash rather than a validation error — the
 * remote host is not a field the participant can correct — so there is no
 * onError branch to write here.
 */
const syncGooglePhoto = () => {
    photoError.value = null;
    localPreview.value = null;
    router.post('/profile/photo/google', {}, photoVisit({}));
};
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5">
                                ...
                                <!--
                                    Sync from Google, offered only when there
                                    is something to fetch — connected *and*
                                    carrying a photo, which the server decides
                                    (google_photo_available) so this cannot
                                    drift from what the endpoint will accept.

                                    It sits with Change and Remove rather than
                                    in the Linked Accounts card next door: this
                                    changes the photo, and the place someone
                                    looks for that is under the photo. The
                                    Google mark travels with it so the word
                                    "Google" is not the only thing tying the
                                    two cards together.
                                -->
                                <button
                                    v-if="user.google_photo_available"
                                    type="button"
                                    class="inline-flex items-center gap-1.5 text-xs font-medium text-csc-blue underline-offset-2 hover:underline disabled:opacity-50"
                                    :disabled="photoBusy"
                                    @click="syncGooglePhoto"
                                >
                                    ...
                                    {{ user.avatar ? 'Sync from Google' : 'Use my Google photo' }}
                                </button>
